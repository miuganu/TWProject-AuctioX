...
function fill_search_auction($text, $user) {
    $query = "SELECT * FROM `products` WHERE user_id=".$user." AND (description LIKE replace('%" . $text . "%',' ', '%' ) OR product_name like replace('%" . $text . "%',' ', '%' ))";
    $result = mysql_query($query);
    return simple_fill_auction($result, $user);
}
...
function simple_fill_auction($result) {
